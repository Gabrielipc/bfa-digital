  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        ...
        const errorResult = err as { status?: number; message?: string };
        if (errorResult.status === 410) {
          navigate({ to: "/sesion-vencida" });
        } else {
          setError(errorResult.message || "Error al validar el codigo de acceso.");
        }
      } finally {
        setLoading(false);
      }
      ...
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md border-0 shadow-lg bg-white">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <div>
              <div className="font-semibold text-primary">Validando acceso a la evaluacion...</div>
              <div className="text-xs text-muted-foreground mt-1">Verificando token y estado de sesion.</div>
            </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md border-0 shadow-lg bg-white">
          <CardContent className="p-8 text-center space-y-4">
            <Brand />
            <div className="text-destructive font-bold text-lg">Error de Validacion</div>
            <p className="text-sm text-muted-foreground">{error}</p>
            <Button className="w-full mt-2" onClick={() => navigate({ to: "/evaluacion/acceso" })}>
              <ArrowLeft className="h-4 w-4 mr-1" /> Reintentar con otro codigo
            </Button>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md border-0 shadow-lg bg-white">
          <CardContent className="p-8 text-center space-y-4">
            ...
            <h1 className="text-xl font-bold text-primary">Aplicacion Pausada</h1>
            <p className="text-sm text-muted-foreground leading-relaxed">
              La sesion de evaluacion ha sido pausada temporalmente por el aplicador. Por favor, espere indicaciones del responsable de laboratorio.
            </p>
            <div className="text-xs text-muted-foreground bg-muted p-2 rounded font-semibold">
              Sesion: {accessData.sessionName}
            </div>
